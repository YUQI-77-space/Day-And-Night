...
### 7.1 Yarn Spinner（对话系统）

#### 步骤一：安装Yarn Spinner

1. 访问Yarn Spinner官方网站（https://yarnspinner.dev/）
2. 下载最新版本的Unity包
3. 在Unity中导入包（Assets → Import Package → Custom Package）

#### 步骤二：配置Yarn Spinner

1. 选择「Tools → Yarn Spinner → Import Templates」
2. 创建对话文件（.yarn文件）
3. 设置对话编译输出路径
4. 配置本地化支持
...
创建一个 `DialogueManager` 脚本，与Yarn Spinner集成：
